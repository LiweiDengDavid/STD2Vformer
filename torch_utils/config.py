...
#TODO Functions to create output directories
def create_output_dir(args):
    if args.output_dir==None:
        # Store the corresponding hyperparameters
        if args.save_log and is_master_process():
            # Find the subscript corresponding to the largest exp and add it to one
        ...
        if not os.path.exists(args.output_dir):
            print('Output path with path {0} does not exist'.format(args.output_dir))
        else:
            shutil.rmtree(args.output_dir)  # Delete all the files in the corresponding folder
...
def base_config(parser):
    # Settings not related to superparameters
...
def get_args(add_config: Callable = None):
    ...
    args = parser.parse_args()  # encapsulate the arguments
